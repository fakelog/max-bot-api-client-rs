...
use crate::{error::MaxBotError, models::{ApiError, BotInfo, BotPatch, Chat, ChatList, ChatPatch, GetSubscriptionsResult, Message, NewMessageBody, SendMessageResult, SubscriptionRequestBody, UpdateList}};
...
impl MaxBotClient {
    ...
    pub async fn get_my_info(&self) -> Result<BotInfo> {
        let response = self.send_request::<()>(reqwest::Method::GET, "/me", None, None).await?;
        Ok(response.json().await?)
    }

    pub async fn edit_my_info(&self, patch: &BotPatch) -> Result<BotInfo> {
        let response = self.send_request(Method::PATCH, "/me", None, Some(patch)).await?;
        Ok(response.json().await?)
    }
    ...
    pub async fn get_chats(&self, count: Option<i32>, marker: Option<i64>) -> Result<ChatList> {
        ...
        let response = self.send_request::<()>(Method::GET, "/chats", Some(&query), None).await?;
        Ok(response.json().await?)
    }
    ...
    pub async fn edit_chat(&self, chat_id: i64, patch: &ChatPatch) -> Result<Chat> {
        let path = format!("/chats/{}", chat_id);
        let response = self.send_request(Method::PATCH, &path, None, Some(patch)).await?;
        Ok(response.json().await?)
    }

    pub async fn delete_chat(&self, chat_id: i64) -> Result<()> {
        let path = format!("/chats/{}", chat_id);
        self.send_request::<()>(Method::DELETE, &path, None, None).await?;
        Ok(())
    }

    // Message methods
    pub async fn send_message(&self, message: &NewMessageBody, chat_id: Option<i64>, user_id: Option<i64>) -> Result<SendMessageResult> {
        let mut query = HashMap::new();
        ...
        let response = self.send_request(Method::POST, "/messages", Some(&query), Some(message)).await?;
        Ok(response.json().await?)
    }
    ...
    pub async fn edit_message(&self, message_id: &str, message: &NewMessageBody) -> Result<()> {
        ...
        self.send_request(Method::PUT, "/messages", Some(&query), Some(message)).await?;
        Ok(())
    }
    ...
    pub async fn delete_message(&self, message_id: &str) -> Result<()> {
        ...
        self.send_request::<()>(Method::DELETE, "/messages", Some(&query), None).await?;
        Ok(())
    }

    // Subscription methods
    pub async fn get_subscriptions(&self) -> Result<GetSubscriptionsResult> {
        let response = self.send_request::<()>(Method::GET, "/subscriptions", None, None).await?;
        Ok(response.json().await?)
    }

    pub async fn subscribe(&self, subscription: &SubscriptionRequestBody) -> Result<()> {
        self.send_request(Method::POST, "/subscriptions", None, Some(subscription)).await?;
        Ok(())
    }
    ...
    pub async fn unsubscribe(&self, url: &str) -> Result<()> {
        ...
        self.send_request::<()>(Method::DELETE, "/subscriptions", Some(&query), None).await?;
        Ok(())
    }
    ...
    pub async fn get_updates(
        ...
        let response = self.send_request::<()>(Method::GET, "/updates", Some(&query), None).await?;
        Ok(response.json().await?)
    }
}